...
function PrintList() {
  ...
  return (
    <ul className={`${classes.searchLists} ${detail.dark && classes.active}`}>
      {detail.data.length > 0 &&
        detail.data.map((item) => (
          <li onClick={() => detail.showBookDetail(item.isbn && item.isbn[0],item.cover_i)} className={detail.dark && classes.active}>
            <img
            ...
            <div className={classes.content}>
              ...
              <p className={classes.writer}>
                by&nbsp;
                {item.author_name &&
                  item.author_name.map((it) => <span>{it}&nbsp;</span>)}
              </p>
